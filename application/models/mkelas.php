...
class Mkelas extends CI_Model{
	...
	function addKelas()
	{	
		$this->db->query("INSERT INTO kelas(nama,tingkat,kode) VALUES('".strip_tags(ascii_to_entities(addslashes($this->input->post('nama',TRUE))))."','".$this->input->post('tingkat',TRUE)."','".$this->input->post('program',TRUE)."')");
	}
	
	function delSiswaKelas($id)
	{
		$kueri = $this->db->query("DELETE FROM catatan WHERE nis='$id' AND id_ta='".$this->session->userdata('kd_ta')."'");
		$kueri = $this->db->query("DELETE FROM industri WHERE nis='$id' AND id_ta='".$this->session->userdata('kd_ta')."'");
		...
		$kueri = $this->db->query("DELETE FROM nilai_diri WHERE nis='$id' AND id_ta='".$this->session->userdata('kd_ta')."'");
		return $kueri;
	}
	...
	function getKelas()
	{
		$query = $this->db->query("SELECT a.id_kelas,a.tingkat,a.kode,a.nama FROM kelas a");
	...
	function getKelasW()
	{
		$query = $this->db->query("SELECT a.id_kelas,a.tingkat,a.kode as kode,a.nama FROM kelas a");
	...
	function getKelase($num,$offset,$sort_by,$sort_order)//menu admin
	{
		...
		$sql = "SELECT id_kelas,nama,tingkat,kode FROM kelas ORDER BY $sort_by $sort_order LIMIT $offset,$num";
		$query = $this->db->query($sql);
		return $query->result();
	}
	...
	function getKelaseW($num,$offset,$sort_by,$sort_order)//menu admin
	{
		...
		$sql = "SELECT a.id_kelas,a.nama as nama,c.nama as wali,b.id_kelas_wali,a.tingkat,a.kode as kode FROM kelas a,kelas_wali b,guru c WHERE b.nip=c.nip AND a.id_kelas=b.id_kelas AND b.id_ta='".$this->session->userdata('kd_ta')."' ORDER BY $sort_by $sort_order LIMIT $offset,$num";
		$query = $this->db->query($sql);
		return $query->result();
	}
	...
	{
		$sql = "SELECT a.id_kelas,a.id_kelas_sis,b.nis,nama,b.tempat_lahir,b.alamat,b.gambar,b.agama,day(b.tanggal_lahir) as tanggal,month(b.tanggal_lahir) as bulan,year(b.tanggal_lahir) as tahun FROM kelas_siswa a,siswa b WHERE a.id_kelas_sis='$id' AND a.nis=b.nis";
		$query = $this->db->query($sql);
		return $query->row();
	}
	...
	{
		$sql = "UPDATE kelas SET nama='".strip_tags(ascii_to_entities(addslashes($this->input->post('nama',TRUE))))."',tingkat='".$this->input->post('tingkat',TRUE)."',kode='".$this->input->post('program',TRUE)."' WHERE id_kelas='".$this->input->post('kode',TRUE)."' ";
		$this->db->query($sql);	
	}
	...
	{
		$kueri = $this->db->query("SELECT * FROM kelas aWHERE a.id_kelas='$id'");
		if($kueri->num_rows() > 0)
		{
			$data = $kueri->row();
	...
	{
		$sql = "UPDATE siswa SET nama='".strip_tags(ascii_to_entities(addslashes($this->input->post('nama',TRUE))))."',alamat='".strip_tags(ascii_to_entities(addslashes($this->input->post('alamat',TRUE))))."',tanggal_lahir='".$this->input->post('tahun').'-'.$this->input->post('bulan').'-'.$this->input->post('hari')."',agama='".$this->input->post('agama',TRUE)."',id_keahlian='$keahlian' WHERE nis='".$this->input->post('nis',TRUE)."' ";
		$this->db->query($sql);	
	}
