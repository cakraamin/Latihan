...
class Mrumus extends CI_Model{
	...
	{
		$sql = "SELECT b.nis,b.nama,b.gambar as nilai FROM kelas_siswa a,siswa b WHERE a.nis=b.nis AND a.id_kelas='$id' AND a.id_ta='".$this->session->userdata('kd_ta')."' ORDER BY b.nis ASC";	
		$query = $this->db->query($sql);
		return $query;
	}
	...
	{
		$query = $this->db->query("SELECT c.id_kelas,c.tingkat,c.kode,c.nama FROM guru_mapel_kelas a,guru_mapel b,kelas c WHERE a.id_mapel=b.id_mapel AND a.id_kelas=c.id_kelas AND b.id_guru_mapel='$id'");
	...
	function addNilai($nip,$id,$kelas,$biji,$tingkat,$ket)
	{
		$kueri = $this->db->query("INSERT INTO nilai(id_guru_mapel,id_kelas,nis,nilai,id_ta,semester,tingkat,ket) VALUES('$id','$kelas','$nip','$biji','".$this->session->userdata('kd_ta')."','".$this->session->userdata('kd_sem')."','$tingkat','$ket')");	
		return $kueri;
	}
	
	function updateNilais($nip,$id,$kelas,$biji,$tingkat,$ket)
	{
		$kueri = $this->db->query("UPDATE nilai SET nilai='$biji' WHERE id_guru_mapel='$id' AND id_kelas='$kelas' AND nis='$nip' AND id_ta='".$this->session->userdata('kd_ta')."' AND semester='".$this->session->userdata('kd_sem')."' AND tingkat='$tingkat' AND ket='$ket'");	
		return $kueri;
	}
	
	function cekNilais($nip,$id,$kelas,$tingkat,$ket)
	{
