...
class Siswa extends CI_Controller {
	...
	function upload()
	{
		...
		else
		{
			...
			for ($i = 1; $i <= $this->spreadsheet_excel_reader->sheets[0]['numRows']; $i++) 
			{
				if($i > 1)
				{
					if($this->spreadsheet_excel_reader->sheets[0]['numCols'] == 7)
					{
						if($this->msiswa->cekNis($this->spreadsheet_excel_reader->sheets[0]['cells'][$i][1]) == 0)
						{
							$value = "'".$this->spreadsheet_excel_reader->sheets[0]['cells'][$i][1]."','".$this->spreadsheet_excel_reader->sheets[0]['cells'][$i][2]."','".$this->spreadsheet_excel_reader->sheets[0]['cells'][$i][3]."','".date('Y-m-d', strtotime($this->spreadsheet_excel_reader->sheets[0]['cells'][$i][4]))."','".$this->spreadsheet_excel_reader->sheets[0]['cells'][$i][5]."','".$this->msiswa->getIdAgama($this->spreadsheet_excel_reader->sheets[0]['cells'][$i][6])."','','".$this->msiswa->getIdTa($this->spreadsheet_excel_reader->sheets[0]['cells'][$i][7])."','".$this->msiswa->getIdJur($this->spreadsheet_excel_reader->sheets[0]['cells'][$i][8])."'";
							//$value = "'".$this->spreadsheet_excel_reader->sheets[0]['cells'][$i][1]."','".strip_tags(ascii_to_entities(addslashes($this->spreadsheet_excel_reader->sheets[0]['cells'][$i][2])))."','".$this->spreadsheet_excel_reader->sheets[0]['cells'][$i][3]."','".date('Y-m-d', strtotime("\"".$this->spreadsheet_excel_reader->sheets[0]['cells'][$i][4]."\""))."','".$this->spreadsheet_excel_reader->sheets[0]['cells'][$i][5]."','".$this->mguru->getIdAgama("\"".$this->spreadsheet_excel_reader->sheets[0]['cells'][$i][6]."\"")."'";
