					...
					$.ajax({
						...
						success: function(html){
							...
							if(obj.statuse == 'gagal'){
								...
								$('#'+obj.newK).html(nilai+'<a id="'+obj.newK+'_2_1" class="next" onclick="remidi(this)"> </a>');
...
$(function(){
	...
	new AjaxUpload(btnUpload, {
		action: '<?=base_url()?>fguru/nilai/upload/<?=$id?>/<?=$kls?>/<?=$tingkat?>/<?=$kes?>',
		name: 'uploadfile',
				...
				$('.tabel_isi').each(function(){
					...
					if(explode[3] == ""){
						$.ajax({
							type: "POST",
							dataType: 'json',
							url: site+"fguru/nilai/cek_nilai/"+explode[0]+"/"+explode[1]+"/"+explode[2]+"/<?=$tingkat?>/<?=$kes?>",
							success: function(html){
								if(html.status == 'ok'){
...
<?php
	if($kls != "" || $kes != "")
	{
		echo '<br/><span id="upload" class="tombolUp">Import Nilai</span>&nbsp;<a href="'.base_url().'fguru/nilai/sample/'.$kls.'">Download</a> contoh file import<br/><br/>';
	}
?>
</div>
