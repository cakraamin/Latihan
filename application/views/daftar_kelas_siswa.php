...
<table width="100%" border="0" class="table1">
	<tr>
		<th width="5%"><input type="checkbox" class="checkall" /></th><th width="10%">Edit</th><th width="25%">NIS</th><th><a href="<?=base_url()?>kelas/daftar/a.nama/<?=(($sort_order == 'asc') ? 'desc' : 'asc')?>">Nama</a></th><th width="20%">Kelas</th>
	</tr>
	<?php
	if(count($kueri) > 0)
	{
	...
	<tr <?=$klas?> id="tabel<?=$dt_kueri->id_kelas_sis?>-<?=$dt_kueri->nis?>">
		<td><?=form_checkbox('cek[]', $dt_kueri->nis)?></td><td><a href="<?=base_url()?>kelas/edit_kelas_siswa/<?=$dt_kueri->id_kelas_sis?>/<?=$ref?>/<?=$alamat?>/<?=$sort_by?>/<?=$sort_order?>/<?=$page?>"><img src="<?=base_url()?>asset/images/edit.png"/></a></td><td><?=$dt_kueri->nis?></td><td><?=$dt_kueri->jeneng?></td><td><?=$dt_kueri->tingkat?> <?=$dt_kueri->kode?> <?=$dt_kueri->kelas?></td>
	</tr>
	<?php
		$no++;
		endforeach;
