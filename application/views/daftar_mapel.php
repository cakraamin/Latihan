...
<table width="100%" border="0" class="table1">
	<tr>
		<th width="5%"><input type="checkbox" class="checkall" /></th><th width="10%">Edit</th><th><a href="<?=base_url().shorting_page($this->uri->uri_string(),$urs,"mapel/".(($sort_order == 'asc') ? 'desc' : 'asc'))?>">Mata Pelajaran</a></th><th>Kelompok Mapel</th>
	</tr>
	<?php
	if(count($kueri) > 0)
	{
	...
	<tr <?=$klas?> id="tabel<?=$dt_kueri->id_mapel?>">
		<td><?=form_checkbox('cek[]', $dt_kueri->id_mapel)?></td><td><a href="<?=base_url()?>mapel/edit_mapel/<?=$dt_kueri->id_mapel?>/<?=$ref?>/<?=$sort_by?>/<?=$sort_order?>/<?=$page?>"><img src="<?=base_url()?>asset/images/edit.png"/></a></td><td><?=$dt_kueri->mapel?></td><td><?=$this->arey->getKelompok($dt_kueri->jenis)?></td>
	</tr>
	<?php
		$no++;
		endforeach;
