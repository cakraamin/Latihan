...
<table width="100%" border="0" class="table1">
	...
	<tr <?=$klas?> id="tabel<?=$dt_kueri->id_mapel_kelas?>">
		<td><?=form_checkbox('cek[]', $dt_kueri->id_mapel_kelas)?></td><td><a href="<?=base_url()?>mapel/edit_guru_mapel_kelas/<?=$kode?>/<?=$dt_kueri->id_mapel_kelas?>/<?=$ref?>/<?=$sort_by?>/<?=$sort_order?>/<?=$page?>"><img src="<?=base_url()?>asset/images/edit.png"/></a></td><td><?=$dt_kueri->tingkat?> <?=$this->arey->getPenjurusan($dt_kueri->kode)?> <?=$dt_kueri->nama?></td>
	</tr>
	<?php
		$no++;
		endforeach;
