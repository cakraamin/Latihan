...
<table width="100%" border="0" class="table1">
	...
	<tr <?=$klas?> id="tabel<?=$dt_kueri->id_kelas_wali?>">
		<td><?=form_checkbox('cek[]', $dt_kueri->id_kelas_wali)?></td><td><a href="<?=base_url()?>kelas/edit_wali/<?=$dt_kueri->id_kelas_wali?>/<?=$ref?>/<?=$sort_by?>/<?=$sort_order?>/<?=$page?>"><img src="<?=base_url()?>asset/images/edit.png"/></a></td><td><?=$dt_kueri->tingkat?> <?=$this->arey->getPenjurusan($dt_kueri->kode)?> <?=$dt_kueri->nama?></td><td><?=$dt_kueri->wali?></td>
	</tr>
	<?php
		$no++;
		endforeach;
